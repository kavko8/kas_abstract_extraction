...
os.makedirs("./povzetki_all_eng", exist_ok=True)
import pandas as pd
        ...
        if a is None:
            if "kas" in name:
                ...
                if abs_index > 0:
                    ...
                    for j, line in enumerate(pdf):
                        if len(line) > 1:
                            ...
                            if similar("keywords", kw, 0.8) or similar("ključnebesede", kb, 0.8) or similar("povzetek", ab, 0.8) or similar("thekeywords:", the_kw, 0.95):
                                break
                ...
                if len(text) > 200:
                    ...
                    with open(f"./povzetki_all_eng/{name.replace('.txt', '-abs-en.txt')}", "w") as f:
